...
def clean_sheet(raw: pd.DataFrame, source: Path, sheet: str) -> tuple[pd.DataFrame | None, str | None]:
    ...
    # Keep only actual records, rather than blank gaps and total/footer lines.
    has_content = frame[BUSINESS_COLUMNS].apply(lambda row: any(not is_blank(v) for v in row), axis=1)
    first_value = frame[BUSINESS_COLUMNS].apply(
        lambda row: next((normalise(v) for v in row if not is_blank(v)), ""), axis=1
    )
    footer = first_value.str.startswith(("total", "sous total", "cumul", "somme"))
    frame = frame[has_content & ~footer].copy()
    if frame.empty:
...
def write_output(frame: pd.DataFrame, output: Path) -> int:
    """Write all rows to a single Excel-compatible workbook, split by worksheet."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--drop-exact-duplicates", action="store_true",
        help="remove identical business rows after consolidation (disabled by default)",
    )
    parser.add_argument(
    ...
    if args.drop_exact_duplicates:
        consolidated = consolidated.drop_duplicates(subset=BUSINESS_COLUMNS, keep="first").reset_index(drop=True)
    parts = write_output(consolidated, output)
    report.update({
        "rows_read": before,
